        ...
        <div class="card">
            ...
            <div class="filters">
                <div class="filter">
                    ...
                    <el-select placeholder="上级部门" v-model="filter.parentDepartmentId" clearable :loading="gettingAllPrisonBureauDepartments">
                ...
                <el-button type="primary" @click="onNew">新增监狱局部门</el-button>
            </div>
            <template>
                ...
                <div class="pagination-box">
                    <span>共{{ pagedPrisonBureauDepartments.totalElements }}条信息</span>
                    <el-pagination
                      @current-change="onPageChange"
                      :current-page.sync="currentPage"
                      :page-size="pagination.size"
                      layout="prev, pager, next, jumper"
                      :total="pagedPrisonBureauDepartments.totalElements">
                    </el-pagination>
                </div>
            </template>
        </div>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      pagedPrisonBureauDepartments: state => state.prisonBureauDepartment.pagedPrisonBureauDepartments
    })
  },
  created() {
  ...
  methods: {
    ...
    onView(id) {
      this.$router.push(`/prison-bureau-department/detail/${id}`);
    },
    onEdit(id) {
      this.$router.push(`/prison-bureau-department/edit/${id}`);
    },
    onDelete(item) {
    ...
    onNew() {
      this.$router.push(`/prison-bureau-department/new`);
    },
    onDeleteConfirm() {
      this.deleting = true;
      this.deletePrisonBureauDepartment(this.deleteItem.id)
        .then(res => {
          this.deleting = false;
