...
export default {
  ...
  data() {
    return {
      form: {
        ...
        criminalSocialRelation: _.cloneDeep(this.$store.state.prisonCriminal.criminalSocialRelation)
      },
      rules: {
  ...
  watch: {
    ...
    'form.selectedPoliticalStatus'(val) {
      this.form.criminalSocialRelation = Object.assign({}, this.form.criminalSocialRelation, {
    ...
    'form.criminalSocialRelation': {
      handler: _.debounce(function(criminalSocialRelation) {
        this.$store.commit('updatePrisonCriminalSocialRelation', criminalSocialRelation);
      }, 500),
      deep: true
    }
  },
  created() {
    Promise.all([criminalLookupService.getAllPoliticalStatuses()]).then(response => {
  ...
  methods: {
    ...
    render() {
      if (!this.criminalSocialRelationId) {
        ...
        this.form.criminalSocialRelation = _.cloneDeep(this.$store.state.prisonCriminal.criminalSocialRelation);
        this.loading = false;
      } else {
        this.getPrisonCriminalSocialRelation(this.criminalSocialRelationId).then(() => {
          this.form.criminalSocialRelation = _.cloneDeep(this.$store.state.prisonCriminal.criminalSocialRelation);
          this.form.selectedPoliticalStatus = {
