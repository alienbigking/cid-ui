...
export default {
  ...
  methods: {
    ...mapActions(['getCriminalFace', 'getCriminalIris', 'getCriminalFingerPrint']),
    render() {
      this.getCriminalFingerPrint(this.$route.params.id)
        .then(() => {
          this.loading = false;
          this.criminalFingerPrint = _.cloneDeep(this.$store.state.prisonCriminal.criminalFingerPrint);
        })
        .catch(() => {
          this.loading = false;
        });
      this.getCriminalFace(this.$route.params.id)
        .then(() => {
          this.loading = false;
          this.criminalFace = _.cloneDeep(this.$store.state.prisonCriminal.criminalFace);
        })
        .catch(() => {
          this.loading = false;
        });
      this.getCriminalIris(this.$route.params.id)
        .then(() => {
          this.loading = false;
          this.criminalIris = _.cloneDeep(this.$store.state.prisonCriminal.criminalIris);
        })
        .catch(() => {
          this.loading = false;
        });
