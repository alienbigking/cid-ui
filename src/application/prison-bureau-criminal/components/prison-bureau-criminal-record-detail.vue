...
export default {
  computed: {
    ...mapState({
      criminalRecord: state => state.prisonBureauCriminal.criminalRecord
    })
  },
  created() {
    this.getPrisonBureauCriminalRecord(this.$route.params.id);
  },
  methods: {
    ...mapActions(['getPrisonBureauCriminalRecord']),
    onBack() {
      this.$router.go(-1);
    }
  }
